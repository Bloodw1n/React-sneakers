...
function App() {
  ...
  const onRemoveItem = (id) => {
    try {
      axios.delete(`https://623ca3ca7efb5abea684d5d3.mockapi.io/cart/${id}`);
      setCartItems((prev) =>
  ...
  const onAddToFavorite = async (obj) => {
    try {
      if (favorites.find((favObj) => Number(favObj.id) == Number(obj.id))) {
        axios.delete(
          `https://623ca3ca7efb5abea684d5d3.mockapi.io/favorites/${obj.id}`
        );
        setFavorites(
          favorites.filter((item) => Number(item.id) !== Number(obj.id))
        );
      } else {
        const { data } = await axios.post(
